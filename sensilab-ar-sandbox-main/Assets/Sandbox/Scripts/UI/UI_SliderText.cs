...
namespace ARSandbox
{
    public class UI_SliderText : MonoBehaviour
    {
        ...
        private string TextPrefix2 = "Value";
        ...
        {   localizationManager = FindObjectOfType<S_LocalizationManager>();
            TextPrefix2 = localizationManager.LocateStringtoDatabase(TextPrefix);
            ...
            if (UseValueStrings)
            {
                ...
                {
                    UI_Text.text = TextPrefix2 + " " + ValueStrings[valueIndex];
                } else
                {
                    UI_Text.text = TextPrefix2 + " " + (sliderValue * ValueScale).ToString("F" + DecimalsToShow.ToString()) + " " + TextSuffix;
                }
            }
            else {
                UI_Text.text = TextPrefix2 + " " + (sliderValue * ValueScale).ToString("F" + DecimalsToShow.ToString()) + " " + TextSuffix;
